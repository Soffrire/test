...
define("LOG_FILENAME", $_SERVER["DOCUMENT_ROOT"] . "/local/components/wbest/sale.order.ajax/order-logs.log");
    ...
    protected function addError($res, $type = 'MAIN')
    {
        ...
        foreach ($errors as $key => $error) {
            ...
            if (
                ...
                $this->arResult['ERROR_SORTED'][$type][$key] = $error;
            }
        }
    }
...
//        die();
        ...
        /**
         * BUG FIX Проверка на существование пользователя,
         * если пользователь есть то авторизуем его и возвращаем его ID
         */
        $clearPhone = str_replace(['(', ')', '+', '-', ' '], '', $userProps['PHONE']);
        $issetUser = $rsUser = CUser::GetByLogin($clearPhone);

        if ($issetUserRs = $issetUser->Fetch()) {
            global $USER;
            $USER->Authorize($issetUserRs['ID']);
            return $issetUserRs['ID'];
        }
        /**
         * BUG FIX END
         */
    ...
    protected function obtainPropertiesForIbElements()
    {
        ...
        foreach ($arResult["BASKET_ITEMS"] as $arBasketItem) {
            ...
            $arBasketItem['DETAIL_PAGE_URL'] = $this->getRealDetailPageUrl($arBasketItem);
            $arBasketItem['OFFERS'] = $this->getProductOffers($arBasketItem, $arCols);
           $mxResult = CCatalogSku::GetProductInfo(
                            $arBasketItem['PRODUCT_ID']
                        );
                        if (is_array($mxResult)) {

                            $res = CIBlockElement::GetProperty(37, $mxResult['ID'], "sort", "asc", array("CODE" => "CML2_ARTICLE"));
                            if ($ob = $res->GetNext()) {

                                 $arBasketItem['PROPERTY_CML2_ARTICLE_VALUE'] = $ob['VALUE'];
                            }
                        }
            $arResult["BASKET_ITEMS"][] = $arBasketItem;
    ...
    protected function getProductOffers($basketItem, $columns)
    {
        $selectProperty = null;
        $propertyCode = null;

        $propertyKeys = ['PROPERTY_RAZMER_KHAR_VALUE', 'PROPERTY_RAZMER_VALUE'];
        foreach ($propertyKeys as $key) {
            if (isset($columns[$key]) && !empty($columns[$key]) && $columns[$key][0]['value']) {

                $propertyCode = $key;
                $selectProperty = $columns[$key][0];
            }
        }
        ...
        $offers = [];
        if (!function_exists('getHigloadOptionValue')) {
            function getHigloadOptionValue($xmlId, $hlblId)
            {
                $entity = \Bitrix\Highloadblock\HighloadBlockTable::compileEntity(\Bitrix\Highloadblock\HighloadBlockTable::getById($hlblId)->fetch());
                ...
                return $data ? $data['UF_NAME'] : null;
            }
        }

        $hlblId = $propertyCode == 'PROPERTY_RAZMER_VALUE' ? 16 : 19;

        $realProductId = $basketItem['PRODUCT_ID'];
        if ($offer = CCatalogSku::GetProductInfo($realProductId))
            $realProductId = $offer['ID'];

        $sample = CCatalogSKU::getOffersList(
            $realProductId,
            null,
            [],
            ['PROPERTY_RAZMER_KHAR', 'PROPERTY_RAZMER_KHAR', 'ID', 'AVAILABLE'],
            $propertyFilter = array()
        );

        $allOffers = $sample ? $sample[$realProductId] : [];
        foreach ($allOffers as $item) {
            if ($item['AVAILABLE'] != 'Y')
                continue;

            $xmlId = $item[$propertyCode];
            $name = getHigloadOptionValue($xmlId, $hlblId);
            $clearPropertyCode = $propertyCode == 'PROPERTY_RAZMER_VALUE' ? 'RAZMER' : 'RAZMER_KHAR';
            $offers[] = [
                'ID' => $item['ID'],
                'NAME' => $name,
                'XML_ID' => $xmlId,
                'CODE' => $clearPropertyCode,
                'INPUT_NAME' => "props[{$clearPropertyCode}]",
                'IS_SELECT' => $selectProperty['value'] == $name
            ];
        }
//		$offers = SortSizesCatalogBasket($offers);
        return $offers;
    }

    protected function getRealDetailPageUrl($basketItem)
    {
        $realProductId = $basketItem['PRODUCT_ID'];
        $offer = CCatalogSku::GetProductInfo($realProductId);

        if (!$offer)
            return $basketItem['DETAIL_PAGE_URL'];

        $realProductId = $offer['ID'];

        $sample = CIBlockElement::GetByID($realProductId);
        $data = $sample->GetNext();

        return mb_substr($data['DETAIL_PAGE_URL'], 0, -1) . $basketItem['DETAIL_PAGE_URL'];
    }
    ...
    protected function obtainUserConsentInfo()
    {
        ...
        $propertyIterator = Sale\Property::getList(array(
            'select' => array('NAME'),
            'filter' => array(
                'ACTIVE' => 'Y',
                'UTIL' => 'N',
    ...
    protected function initDelivery(Shipment $shipment)
    {
        ...
        if (!empty($this->arDeliveryServiceAll)) {
            if (isset($this->arDeliveryServiceAll[$deliveryId])) {
                ...
                $deliveryObj = $this->arDeliveryServiceAll[3];
                if (!empty($deliveryId)) {
    ...
    protected function getInnerPaySystemInfo(Order $order, $recalculate = false)
    {
        ...
        if ($this->arParams['PAY_FROM_ACCOUNT'] === 'Y' && $order->isAllowPay()) {
            ...
            if (!$innerPayment) {
                return array(0, $arPaySystemServices);
            }
    ...
    protected function getJsDataResult()
    {
        ...
        $result['BONUS']['AVAILABLE_WRITE_OFF'] = $this->bonusAvailableWriteOff();
        $result['BONUS']['BONUS_PAY_COUNT'] = $this->request->get('BONUS_PAY_COUNT') ?: 0;

        if ($this->request->get('BONUS_PAY_SEND_SMS_CODE') === 'Y'):
            if ($this->request->get('BONUS_PAY_COUNT') > 0) {
//            $response = (new \App\Bonus\Controller\ConfirmationCode())->sendWriteOffConfirmationCode();
//
//            if ($response->success) {
//                $result['BONUS']['BONUS_PAY_SMS_CODE_SENT'] = 'Y';
//            } else {
//                $result['BONUS']['BONUS_PAY_SMS_CODE_SENT'] = 'N';
//
//                if ($response->error_description) {
//                    $result['BONUS']['BONUS_PAY_SMS_CODE_SENT']['ERROR_MESSAGE'] = $response->error_description;
//                } else {
//                    $result['BONUS']['BONUS_PAY_SMS_CODE_SENT']['ERROR_MESSAGE'] = 'При отправке СМС кода произошла ошибка';
//                }
//            }

                $result['BONUS']['BONUS_PAY_SMS_CODE_SENT']['STATUS'] = 'Y';
                $result['BONUS']['BONUS_PAY_SMS_CODE_SENT']['ERROR_MESSAGE'] = 'При отправке СМС кода произошла ошибка';
            } else {
                $result['BONUS']['BONUS_PAY_SMS_CODE_SENT']['STATUS'] = 'N';
                $result['BONUS']['BONUS_PAY_SMS_CODE_SENT']['ERROR_MESSAGE'] = 'Введите количество бонусов для списания';
            }
        endif;

        if (!empty($this->request->get('BONUS_PAY_SMS_CODE'))):
//            $response = \App\Bonus\Controller\ConfirmationCode::verifyConfirmationCode([
//                'phone' => \Bitrix\Main\Engine\CurrentUser::get()->getLogin(),
//                'code' => $this->request->get('BONUS_PAY_SMS_CODE')
//            ]);
//
//            if ($response->success) {
//                $result['BONUS']['BONUS_PAY_SMS_CODE_CHECK']['STATUS'] = 'Y';
//            } else {
//                $result['BONUS']['BONUS_PAY_SMS_CODE_CHECK']['STATUS'] = 'N';
//
//                if ($response->error_description) {
//                    $result['BONUS']['BONUS_PAY_SMS_CODE_CHECK']['ERROR_MESSAGE'] = $response->error_description;
//                } else {
//                    $result['BONUS']['BONUS_PAY_SMS_CODE_CHECK']['ERROR_MESSAGE'] = 'Проверочный код из СМС неправильный';
//                }
//            }
            ...
            //$result['BONUS']['BONUS_PAY_CONFIRM']['STATUS'] = 'Y';
            //$result['BONUS']['BONUS_PAY_CONFIRM']['ERROR_MESSAGE'] = 'Ошибка в применение бонусов';

            $this->addBonusProduct(100);
        endif;
     ...
     */
    protected function bonusAvailableWriteOff()
    {
        $bonusAvailableWriteOff = 0;

        /** @var Sale\BasketItem $item */
        foreach ($this->order->getBasket() as $key => $item) {
            if ($item->canBuy() && !$item->isDelay()) {
                $bonusAvailableWriteOff += ($item->getFinalPrice() / 100) * 20;
            }
        }

        return $bonusAvailableWriteOff;
    }

    protected function addBonusProduct($bonus)
    {
        $order = $this->order;
        ...
        foreach ($order->getBasket() as $basketItem) {
            if ($basketItem->canBuy() && !$basketItem->isDelay()) {
                if ($bonus) {
                    $bonusAvailableWriteOff = 0;
                    $bonusAvailableWriteOff += ($basketItem->getFinalPrice() / 100) * 20;

//                    if ($bonus > $bonusAvailableWriteOff) {
//                        $basketItem->setField('PRICE', $bonusAvailableWriteOff);
//                        $bonus = $bonus - $bonusAvailableWriteOff;
//                    } else {
//                        $basketItem->setField('PRICE', $bonus);
//                        $bonus = 0;
//                    }
                    ...
                    /** @var \Bitrix\Sale\BasketPropertyItem $propertyItem */
                    foreach ($propertyCollection as $propertyItem) {
                        if ($propertyItem->getField('CODE') === 'BONUSPAY') {
                            $propertyItem->setField('VALUE', $bonus);
                            $propertyItem->save();
                            break;
                        }
                    }
        ...
        $this->order->setBasket($order->getBasket());
    }
    ...
    protected function processOrderAction()
    {
        ...
        // AddMessage2Log($time, 'processOrderAction');
    }
	...
	public function executeComponent()
    {
		...
		$this->Log($isAjaxRequest?"is_ajax_post":"none ajax post");
	...
	private function Log($str)
	{
		global $USER;
		$idUser = $USER->GetID();
		$str = date('Y-m-d H:i:s') . "-" . $idUser . ': ' . $str;
		file_put_contents(__DIR__ . '/basket_logs.txt', $str . PHP_EOL, FILE_APPEND);
	}
}
